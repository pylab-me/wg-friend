...
pub fn add(
    ...
    print_kv("server_config", iface.conf_file.display().to_string());
    print_kv("client_config", client_file.display().to_string());
    Ok(())
}
...
pub fn remove(app: &AppConfig, interface: Option<String>, name: Option<String>) -> Result<()> {
    ...
    println!("Removed client {name} from {}.", iface.interface);
    Ok(())
}
...
pub fn export(
    ...
    print_header("client export");
    print_kv("server", &iface.interface);
    print_kv("name", &name);
    print_kv("source", source.display().to_string());
